#!/usr/bin/env python3
"""
USA RD Fetcher - Fetches USA RD players and merges with existing USA data
"""
...
def fetch_usa_rd_players():
    """
    Fetch all USA RD players
    """
    
    print("=== USA RD Player Fetcher ===")
    print("Target: Fetch all USA players with RD position")
    print()
    
    # Step 1: Get all USA RD players
    print("Step 1: Fetching USA RD players...")
    ...
    print(f"Found {len(rows)} USA RD players (total available: {total})")
    ...
    print(f"\nStep 2: Cleaning {len(all_players)} USA RD players...")
    ...
    for player in cleaned_players:
        ...
        if (nationality.upper() == 'USA' and 
            position == 'RD'):
            usa_rd_players.append(player)
    
    print(f"Final USA RD players: {len(usa_rd_players)}")
    return usa_rd_players

def merge_usa_players_with_rd():
    """
    Merge USA RD players with existing USA players
    """
    
    print("\n=== Merging USA Players with RD ===")
    
    # Load existing USA players (LW + RW + LD)
    try:
        with open('usa.json', 'r') as f:
            existing_players = json.load(f)
        print(f"Loaded {len(existing_players)} existing USA players (LW + RW + LD)")
    except FileNotFoundError:
        print("No existing usa.json found")
        existing_players = []
    
    # Fetch USA RD players
    usa_rd_players = fetch_usa_rd_players()
    
    # Merge all USA players
    all_usa_players = existing_players + usa_rd_players
    ...
    print(f"  - RD players: {len(usa_rd_players)}")
...
def main():
    print("Starting USA RD Fetcher and Merger...")
    print("=" * 60)
    
    all_players = merge_usa_players_with_rd()
    
    print(f"\n🎯 FINAL RESULT:")
    print(f"Total USA players (LW + RW + LD + RD): {len(all_players)}")
