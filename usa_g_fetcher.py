#!/usr/bin/env python3
"""
USA G Fetcher - Fetches USA G players and merges with existing USA data
"""
...
def fetch_usa_g_players():
    """
    Fetch all USA G players
    """
    
    print("=== USA G Player Fetcher ===")
    print("Target: Fetch all USA players with G position")
    print()
    
    # Step 1: Get all USA G players
    print("Step 1: Fetching USA G players...")
    ...
    print(f"Found {len(rows)} USA G players (total available: {total})")
    ...
    print(f"\nStep 2: Cleaning {len(all_players)} USA G players...")
    ...
    for player in cleaned_players:
        ...
        if (nationality.upper() == 'USA' and 
            position == 'G'):
            usa_g_players.append(player)
    
    print(f"Final USA G players: {len(usa_g_players)}")
    return usa_g_players

def merge_usa_players_with_g():
    """
    Merge USA G players with existing USA players
    """
    
    print("\n=== Merging USA Players with G ===")
    ...
    # Fetch USA G players
    usa_g_players = fetch_usa_g_players()
    
    # Merge all USA players
    all_usa_players = existing_players + usa_g_players
    ...
    print(f"  - G players: {len(usa_g_players)}")
...
def main():
    print("Starting USA G Fetcher and Merger...")
    print("=" * 60)
    
    all_players = merge_usa_players_with_g()
