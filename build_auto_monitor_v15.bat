...
echo Building NHL Card Monitor Auto v15 with improved missing cards detection...
...
if %BUILD_RESULT% equ 0 (
    ...
    echo   - IMPROVED missing cards detection:
    ...
    echo     * Stop after finding 1000 missing cards to prevent infinite loop
    echo     * Stop after checking 20 pages if no missing cards found
    echo     * This fixes the issue where new cards are on page 11+ but program stops at page 1
    echo   - Enhanced debug logging for missing URLs detection
    echo   - Automatic monitoring and updating
    echo.
    echo 📊 Now with IMPROVED missing cards detection!
    echo.
    pause
) else (
    echo.
