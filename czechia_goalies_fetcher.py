#!/usr/bin/env python3
"""
Czechia Goalies Fetcher
Fetches Czechia goalies separately and merges with existing data
"""
...
def fetch_goalies_with_timeout(nationality, max_retries=3, timeout=30):
    ...
    print(f"🥅 Fetching Czechia goalies...")
...
def run_czechia_goalies_fetcher():
    """Main function to fetch Czechia goalies and merge with existing data"""
    print("🇨🇿 CZECHIA GOALIES FETCHER")
    print("=" * 50)
    
    # Load existing skaters
    try:
        with open('czechia.json', 'r', encoding='utf-8') as f:
            existing_players = json.load(f)
    except FileNotFoundError:
        print("❌ czechia.json not found! Run czechia_complete_fetcher.py first.")
        return
    ...
    # Fetch goalies
    print("\n🥅 FETCHING GOALIES")
    goalies = fetch_goalies_with_timeout('Czechia')
