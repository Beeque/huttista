...
def fetch_player_data(player_id, is_goalie=False):
    """Hae pelaajan tiedot DataTables API:lla"""
    ...
    # Filtteröi player_id:n mukaan
    if is_goalie:
        # Maalivahdeille ei ole suoraa player_id saraketta, joten käytetään full_name saraketta
        payload['columns[10][search][value]'] = str(player_id)
    else:
        # Kenttäpelaajille ei myöskään ole suoraa player_id saraketta
        payload['columns[11][search][value]'] = str(player_id)
    ...
    try:
        ...
        if players_data:
            return players_data[0]  # Palauta ensimmäinen (ja ainoa) pelaaja
        else:
            print(f"⚠️ Ei löytynyt dataa player_id:lle {player_id}")
            return None
