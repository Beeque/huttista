...
echo Building NHL Card Monitor Auto v18 with performance optimizations...
...
if %BUILD_RESULT% equ 0 (
    ...
    echo     * Removed debug logging messages
    echo     * Reduced page delay from 1.0s to 0.5s
    echo     * Reduced retry delay from 1.0s to 0.5s
    echo     * Added concurrent fetching with ThreadPoolExecutor (5 workers)
    echo     * Significantly faster card details fetching
    ...
    echo.
    echo 📊 Now with PERFORMANCE OPTIMIZATIONS!
    echo.
    pause
) else (
    echo.
