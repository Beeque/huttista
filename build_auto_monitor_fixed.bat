@echo off
chcp 65001 >nul
title NHL Card Monitor - Build Auto Version (Fixed)
echo ========================================
echo    NHL Card Monitor - Build Auto (Fixed)
echo ========================================
echo.
echo Building AUTO version with FIXED player name parsing...
echo.
...
echo.
echo Building AUTO GUI executable with FIXED player name parsing...
echo.
...
if exist "dist\NHL_Card_Monitor_Auto.exe" (
    echo Removing old executable...
    del /f /q "dist\NHL_Card_Monitor_Auto.exe" >nul 2>&1
)
...
echo   • Fixed player name parsing (no more "Player Stat")
echo   • Better name extraction from HTML
echo   • Filters out generic titles
echo   • Handles both skaters and goalies correctly
echo.
pause