#!/usr/bin/env python3
"""
Lopullinen NHL HUT Builder Web Scraper
Käyttää curl:ia ja tallentaa sivun sisällön analyysiä varten
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('final_nhl_scraper.log'),
...
class FinalNHLScraper:
    def __init__(self, db_path: str = 'nhl_cards_final.db'):
    ...
    def get_page_content(self, url: str, save_to_file: bool = True) -> Optional[str]:
        """Hae sivun sisältö curl:lla ja tallenna tiedostoon"""
        try:
            ...
            if result.returncode == 0:
                ...
                # Tallenna sivun sisältö tiedostoon analyysiä varten
                if save_to_file:
                    player_id = url.split('id=')[1] if 'id=' in url else 'unknown'
                    filename = f"page_content_{player_id}.html"
                    with open(filename, 'w', encoding='utf-8') as f:
                        f.write(html_content)
                    logger.info(f"Sivun sisältö tallennettu tiedostoon: {filename}")
    ...
    def scrape_single_card(self, player_id: int) -> Optional[PlayerCard]:
        ...
        # Hae sivun sisältö
        html_content = self.get_page_content(card_url, save_to_file=True)
        if not html_content:
            return None
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Final NHL HUT Builder Web Scraper')
